...
def parse_range(s):
    ...
    # Пометка am/pm указана один раз в конце — она относится к обеим границам.
    # «5.00-11.00pm» это 17:00-23:00, а не «с пяти утра»; «12.00-3.00pm» —
    # это полдень-15:00. Прежнее правило («считаем утро») давало заведениям
    # открытие на двенадцать часов раньше настоящего.
    if not ap1 and ap2:
        ap1 = ap2
    a = to24(h1, m1, ap1); b = to24(h2, m2, ap2)
    if not a or not b: return None
    # Полночь как время закрытия: 00:00 читается как нулевая длительность.
    if b == '00:00': b = '23:59'
    # Закрытие после полуночи (бар до 2-4 утра). Календарного способа
    # выразить это в одной строке нет — обрезаем до конца суток, чтобы не
    # утверждать, что заведение открывается в 20:00 и закрывается в 04:00
    # того же дня.
    if b < a: b = '23:59'
    return f"{a}-{b}"
