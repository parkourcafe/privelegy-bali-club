(function () {
  ...
  if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.App) {
    ...
    appPlugin.addListener("appUrlOpen", function (event) { var kind = applyDeepLink(event.url); render(); if (kind === "plan") revealPlan(); });
    if (appPlugin.getLaunchUrl) appPlugin.getLaunchUrl().then(function (event) { if (event && event.url) { var kind = applyDeepLink(event.url); render(); if (kind === "plan") revealPlan(); } }).catch(function () {});
  }
  render();
})();
