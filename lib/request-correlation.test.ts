...
import { publicErrorReference } from "./public-error-reference";
...
test("public error references expose only bounded digest and generated request IDs", () => {
  assert.deepEqual(publicErrorReference({ digest: "82910442", requestId: FIRST_ID }), {
    digest: "82910442",
    requestId: FIRST_ID,
  });
  assert.deepEqual(publicErrorReference({
    digest: "secret with spaces",
    requestId: "attacker-controlled",
  }), { digest: null, requestId: null });
});
