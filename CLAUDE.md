...
- [x] **Canggu editorial pass tranche 2** shipped (2026-07-12, same branch, migration `0022_canggu_editorial_pass_t2.sql`; **prod apply = founder step, pending**). Verified web-research pass over the remaining active Canggu catalogue → **50 more venues** enriched & publishing (Batu Bolong/Berawa/Pererenan/Seseh/Cemagi restaurants, remaining cafés, one wine bar, wellness/spa/beauty/fitness). Essentially the whole active Canggu catalogue now carries editorial. Left for founder review: `dandelion` (rebranded → Casa Tua, needs rename not fill) and `dicarik-warung` (ambiguous, no first-party Canggu match). Optional cleanup: 3 sport/fitness rows (`jungle-padel`, `wrong-gym`, `finns-recreation-club`) are keyed `category=spa`. Photos still absent → detail pages stay noindex.
