...
- [x] **Sanur + Seminyak thin-page editorial pass** shipped (2026-07-13, branch `claude/privelegy-bali-club-mjlzwq`, migration `0026_enrich_thin_venues.sql`, applied to prod). Closed the last empty active venue rows outside the deep districts: a fresh verified web-research pass (6 parallel research agents, first-party sites/IG) on **38 thin venues — 20 Sanur + 13 Seminyak food/drink + 5 Umalas wellness** → `why_its_here` + `best_for` + `not_for` + `what_to_order`/price bands + `jobs`. **37/38 now clear the decision-ready bar** and their `/places/[slug]` pages flip to `index,follow` + enter the sitemap (`isVenueIndexable`, all districts). Held back honestly: `revive-pilates-umalas` — real editorial written but no verifiable class price band, so it stays noindex until a band is confirmed. Guardrails held (no ratings/review counts #1; downsides only as `not_for` fit-context #7; prices as bands; Sanur = sunrise coast so no `sunset_drinks_view`; unverified fields left blank, nothing invented). `white-rock-beach-club` (the 39th thin page) is Uluwatu — gated by the evidence registry, not DB editorial — so it's out of scope here and still needs a `lib/uluwatu/venues.ts` entry. Detail pages carry no photos yet, but the publication gate does not require them for these districts. Docs: `docs/sanur-seminyak-editorial-pass.md`.
- [x] **Seminyak editorial evidence pass (full active catalogue)** shipped (2026-07-13, branch `claude/other-bali-positioning-n60hpl`, migration `0027_seminyak_editorial_pass.sql`, applied to prod). Ran in parallel with the thin-page pass above; both are fill-empties/idempotent so they compose cleanly. Verified cycle over the 40 empty active Seminyak venues (food/drink, spa, beauty, fitness, yoga). **33 enriched + 7 cleaned up → Seminyak 18 → 51/51 active publishable.** Cleanup: collapsed Soham (×4→1 `soham-wellness-center`), Prana yoga (×2→1), Bodyworks (×2→1) duplicates; deactivated mis-districted `cocoon-medical-spa` (Legian) & `rejuvie-aesthetic` (Kuta/Sanur). Uses existing `beauty`/`fitness`/`yoga` categories (no new entity #11); planning_only, no money loop (#4). Followed by the hand-crafted `/seminyak` pillar + guides (this branch). Docs: `docs/seminyak-editorial-pass.md`.
