...
import { KNOWN_DISTRICTS, VENUE_TYPES } from "../../lib/schema/venue";
...
function googleMapsUrl(value: unknown, field: string): string {
  ...
  if (!safe) throw new Error(`${field} must be a supported Google Maps HTTPS URL`);
  return safe;
}
...
export function parseMobileBootstrap(value: unknown): MobileBootstrapPayload {
  ...
  if (data.consentVersion !== CONSENT_VERSION) throw new Error("Unsupported consent version");
