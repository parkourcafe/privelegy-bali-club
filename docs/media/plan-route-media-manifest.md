...
- Model: Higgsfield `gpt_image_2`; generated 2026-07-24 UTC.
- Batch cost: 56.00 credits; account balance moved from 673.69 to 617.69.
- Eight source PNGs at 2336 × 1744 were visually reviewed as one contact sheet.
- Human/anatomy risk frames (`food`, `sunset`, `heritage`) received an additional full-resolution review.
- All eight passed checks for route relevance, Bali specificity, anatomy, text/logos, identifiable businesses, false named-place implication and overlay crop safety.
- Shipped assets are 1200 × 896 WebP at quality 68 with encoder effort 6.
- Final weights are 68–108 KB per image.
- Exact prompt composition, job IDs and source filenames are recorded in `docs/media/higgsfield-plan-route-batch.json`. Ephemeral source URLs are intentionally omitted because their CDN paths contain an account-scoped segment.
- `scripts/fetch-scenes.mjs` reproduces any missing WebP from the recorded source filenames; fetch failure remains non-blocking and falls back to the existing generated SVG art.
