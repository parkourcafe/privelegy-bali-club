...
## Статус на конец дня 2026-08-04

**Задачи 1–3 выполнены** — прогнаны напрямую в базе, результат ниже. Задача 4
остаётся, она вынесена в отдельное ТЗ: `docs/TZ_DESCRIPTIONS_GROUP_A.md`.
...
Опубликованных заведений: 1244 → **1221**.

Дубли не удалены, а переведены в `review`. У всех 23 пар живой оказалась
запись со slug, содержащим район (создана 13 июля); голая (20 июля) была
пустым повторным импортом. Перед снятием проверено построчно, что ни одно
поле не теряется — ни часов, ни координат, ни ссылок.

Три пары не попали под общее правило, потому что название заведения само
содержит район (`f45-training-canggu`, `jungle-padel-canggu`,
`bali-fitness-seminyak`), и закрыты отдельно.

Ниже — исходное задание, оставлено как запись того, что и почему делалось.
...
> **Поправка по факту выполнения.** Таблицы ниже угадывали, какую запись из
> пары оставлять, и угадали в большинстве случаев **наоборот**. Запрос шага 1
> показал устойчивую картину: живая запись — та, где район входит в slug
> (создана 13 июля, с описанием, фото и «для кого»), а голая (20 июля) была
> пустым повторным импортом. Оставлены записи с районом.
>
> Это ровно то, ради чего в шаге 1 стоит запрос, а не глаз: список ниже —
> вход для проверки, и проверка его переписала.
