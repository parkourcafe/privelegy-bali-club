...
## Review record (2026-07-15)

Reviewed against `AGENTS.md` and `Other_Bali_Master_Architecture.md` and
**approved**: four-surface topology (§5), no duplicated reservation table
(guardrail #4, §2.2), TablePilot aggregate-only bookings with no guest PII
(§15.3, §16), independent operator-review / publication / owner-confirmation
states plus a distinct photo rights/consent gate (§11, §17; AGENTS.md content
publication rule v2), authorization sourced from a server-controlled venue
membership + RLS rather than editable `user_metadata` (§16.2), and draft-only
partner writes that cannot touch Other Bali editorial fields (§9.3). The P0→P2
fix order is endorsed. This document is a design checkpoint only; production
migration apply remains an explicit founder step (§21).
